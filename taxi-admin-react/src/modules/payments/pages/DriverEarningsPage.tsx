...
import { getDriverEarningsRows, getDriverEarningsSummary } from '../services/payments.service'
import type { DriverEarningsRow, SummaryMetrics } from '../types/payments.types'
...
function DriverEarningsPage() {
  ...
  useEffect(() => {
    void getDriverEarningsRows().then(setRows)
    void getDriverEarningsSummary().then(setSummary)
  }, [])
    ...
    <div className="flex h-screen bg-taxi-bg">
      ...
      <div className="h-screen flex-1 overflow-y-auto" style={{ marginLeft: layout.isMobile ? 0 : 256 }}>
        <Navbar
          ...
          subtitle="Track gross, commission and net earnings"
        />
        <main className="space-y-6 p-4 lg:p-6">
          ...
          {summary ? (
            <RevenueStatsCards
              items={[
                { label: 'Total Driver Earnings', value: summary.total },
                { label: "Today's Earnings", value: summary.today },
                { label: 'This Month Earnings', value: summary.month },
              ]}
            />
          ) : null}
          <DriverEarningsTable rows={rows} />
        </main>
      </div>
    </div>
  )
