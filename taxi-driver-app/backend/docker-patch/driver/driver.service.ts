...
import { validateDriverProfile } from "../driver-profile/driver-profile.service";
...
export const setDriverRegistrationPassword = async (email: string, password: string) => {
  ...
  const token = generateAccessToken(driver.id, "DRIVER");
  return {
    token,
    user: {
      id: driver.id,
...
export const updateDriverStatus = async (
  ...
  if (status === "ONLINE") {
    ...
    await validateDriverProfile(driver.id);
  }
